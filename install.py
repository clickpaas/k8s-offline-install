...
def main():
    ...
    map(lambda _: bootstrap_enviroment(_.get("ip"), _.get("password")), all_nodes)
    ...
    if network_list.get_code() != network_list.Success:
        ...
        _k8sapi_ip_or_index = raw_input(ColorPrompt.ensure_show(
        ...
        if get_ip_address:
            ...
            api_server_addr = networks_info.values()[int(_k8sapi_ip_or_index)]
    ...
    data_plane_interface = raw_input(
    ...
    data_iface = networks_info.keys()[int(data_plane_interface)]
    ...
    network_plugin = int(raw_input(ColorPrompt.prompt("Enter you chose[1|2]: ")))
    ...
    for node in hosts.get("nodes"):
        ...
        host, passwd = node.get("ip"), master.get("password")
    ...
    if pre_installed_kubeadm.lower() == 'y':
        ...
        map(lambda _: RemoteCommand.security_command(_.get("ip"), _.get("password"), command), all_nodes)

    # install docker for all nodes
    print(ColorPrompt.stage_info("Install docker stage"))
    map(lambda _: DockerInstall.install_from_binary(_.get("ip"), _.get("password")), all_nodes)

    # install kubeadm for all nodes

    print(ColorPrompt.stage_info("Install kubeadm packages stage"))
    # install k8s rpm
    print(ColorPrompt.stage_info("begin install kubernetes rpm"))
    map(lambda _: KubernetesInstall.install_from_rpm(_.get("ip"), _.get("password")), all_nodes)
    # load images
    print(ColorPrompt.stage_info("begin load k8s some docker images to host"))
    map(lambda _: KubernetesInstall.load_images(_.get("ip"), _.get("password")), all_nodes)
    ...
    init = getstatusoutput('kubeadm init  --kubernetes-version={} --pod-network-cidr={}  --control-plane-endpoint {} '
    ...
    # # if config.gpus:
    # #     print(stage_info("Apply nvidia plugin for k8s"))
    # #     map(lambda _: nvidia_docker_config(_.split("/")[0], _.split("/")[1]), config.gpus)
    #
    # print(ColorPrompt.stage_info("Clean all temp files  stage"))
    # post_install.clearn_all_package()
