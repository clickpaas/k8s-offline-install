...
import sys
from common.utils import ColorPrompt, get_project_root_path
...
def load_images(host, password, role):
    ...
    map(lambda _: RemoteCommand.security_command(host, password, "docker load -i {}".format(_)), images)
...
def scp_packages_to_nodes(host, password):
    ...
    map(lambda rpm: LocalCommand.scp(host, password,
